...
if [ "$zenity_version" = "" ]; then
    echo
    echo ===================================================================
    echo "你没有安装 zenity,无法显示图形界面,将使用命令行界面."
    echo "可以通过软件包管理器安装 zenity,如 sudo apt install zenity"
    echo "不同发行版命令不同."
    echo ===================================================================
    echo
...
text=$(
    cat <<END_HEREDOC
请在弹出的对话框中选择要加速的游戏程序 (如 Wow.exe).

注意：应该选择实际游戏进程 (比如 Wow.exe),
不是游戏启动器 (比如 Battle.net.exe).
...
text=$(
    cat <<END_HEREDOC
即将加速 $exe_name
为保证加速效果, Linux 上的 IPv6 功能将被暂时关闭,重启后可恢复.
...
注意：在你双击桌面上的 "启动代理.bat" 之前,你的 Linux 系统将无法上网,
只有 Windows 虚拟机可以上网.

要停止加速器,请给 Win10 虚拟机关机,或者在 Linux 终端里按 Ctrl+C.
END_HEREDOC
)
...
sudo -A "$pwd/linux/jiasuqi" "$VM_IP_ADDR" "$exe_name" || {
    zenity --info --text="sudo 密码错误,加速器无法启动" --width=300 2> /dev/null
    exit
}
